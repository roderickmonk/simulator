...
if __name__ == "__main__":
    ...
    try:
        ...
        simulation = sim_db.simulations.find_one({"_id": ObjectId(sim_id)})
        ...
        logging.info("{0:30}{1:10}".format("Actual Start:", str(actual_start)))
        logging.info("{0:30}{1:10}".format("Actual End:", str(actual_end)))
        ...
        for i in range(len(partitions) - 1):
            ...
            if optimized:
                ...
                if os.environ.get("SIM_DEBUG"):
                    ...
                    command = ["python", path, str(partition_id)]

                else:

                    command = ["python", "-O", path, str(partition_id)]
        ...
        rounded_end = "{0:.1f}".format(round(end_simulation - start_simulation, 4))
        ...
        logging.info("{0:24}{1:8d}".format("Total Sim Trades:", number_sim_trades))
